...
// postcondition: returns value of bigint + rhs after addition
{
    ...
    unsigned k;
    unsigned len = NumDigits();         // length of larger addend
...
unsigned BigInt::NumDigits() const
// postcondition: returns # digits in BigInt
{
    return myNumDigits;
}

int BigInt::GetDigit(unsigned k) const
...
void BigInt::ChangeDigit(unsigned k, int value)
// precondition: 0 <= k < NumDigits()
